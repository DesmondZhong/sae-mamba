...
- **Mamba-2 activation capture (known issue)**: The mamba_ssm `Block.forward` returns `(mixer_output, residual_before_block)`. The original extraction hook captured `output[0]` only, which is the per-layer SSM *contribution*, not the post-block residual stream (= `output[0] + output[1]`). All Mamba-2 rows in the FVE / K-sweep / expansion-sweep tables reflect SAEs trained on per-layer mixer output and are **not directly comparable** to the Mamba-1 and Pythia rows (trained on the residual stream). The capture hook has been corrected in the codebase; Mamba-1 and Pythia numbers are unaffected and continue to support the headline findings (PR gap, causal leverage, induction, PPL blow-up, SAE CKA).
